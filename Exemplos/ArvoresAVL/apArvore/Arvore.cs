...
public class Arvore<Dado> where Dado : IComparable<Dado>
{
  ...
  public Arvore(PictureBox ondeExibir)
  {
    raiz = atual = antecessor = null;
    quantosNos = 0;
    this.ondeExibir = ondeExibir;
  }
  ...
  public NoArvore<Dado> Insert(Dado item, NoArvore<Dado> n)
  {
    if (n == null)
      n = new NoArvore<Dado>(item); // atributo altura valerá 0!
    else
    {
  ...
  private NoArvore<Dado> RotateWithLeftChild(NoArvore<Dado> no)
  {
    NoArvore<Dado> temp = no; // apenas para declarar
    temp = no.Esq;
    no.Esq = temp.Dir;
    temp.Dir = no;
    ...
    MessageBox.Show("Rotação à direita do nó " + temp.Info.ToString());
    ondeExibir.Invalidate();
    Application.DoEvents();
    return temp;
  }
  private NoArvore<Dado> RotateWithRightChild(NoArvore<Dado> no)
  {
    NoArvore<Dado> temp = no; // apenas para declarar
    temp = no.Dir;
    no.Dir = temp.Esq;
  ...
  private NoArvore<Dado> DoubleWithLeftChild(NoArvore<Dado> no)
  {
    ondeExibir.Invalidate();
    MessageBox.Show("Rotação dupla à direita do nó " + no.Info.ToString());
    no.Esq = RotateWithRightChild(no.Esq);
    ondeExibir.Invalidate();
    Application.DoEvents();
    return RotateWithLeftChild(no);
  }
  private NoArvore<Dado> DoubleWithRightChild(NoArvore<Dado> no)
  {
    ondeExibir.Invalidate();
